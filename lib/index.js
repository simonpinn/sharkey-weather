...
app.get('/', function(req, res){
    ...
    fetchData(connectionString, native, "SELECT * FROM wind where created_date > now() - interval '12 hour'", (data) => {
        windData = data;
        fetchData(connectionString, native, "SELECT * FROM temperatureHumidity where created_date > now() - interval '12 hour'", (data)=> {
           tempData = data;
           fetchData(connectionString, native, "SELECT * FROM rain where created_date > now() - interval '12 hour' ORDER BY created_date DESC LIMIT 50", (data)=> {
               rainData = data;
